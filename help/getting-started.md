---
title: 开始使用Adobe Marketo限定符
description: 了解如何在团队开始使用应用程序之前完成Adobe Marketo限定符的一次性管理员设置，包括用户组和CRM连接。
...
# 开始使用Adobe Marketo限定符

Adobe为您的组织设置Marketo限定符后，[!DNL Marketo]系统管理员必须创建所需的用户组并连接Salesforce或Microsoft Dynamics 365。

![Marketo限定符主页](assets/homepage.png){width="800" zoomable="yes"}
...
1. 打开&#x200B;**[!UICONTROL 用户]**&#x200B;并选择&#x200B;**[!UICONTROL 添加用户]**&#x200B;以添加需要访问Marketo限定符的所有人。

### Marketo Qualifier管理员

配置CRM连接、[知识中心](admin-settings.md#knowledge-center)和全局电子邮件选择退出设置的管理员也必须属于`Marketo Qualifier Admins`用户组。

1. 在Adobe Admin Console中，选择&#x200B;**[!UICONTROL 用户]** > **[!UICONTROL 用户组]** > **[!UICONTROL 新用户组]**。
1. 输入`Marketo Qualifier Admins`作为组名，然后选择&#x200B;**[!UICONTROL 保存]**。
1. 打开&#x200B;**[!UICONTROL 用户]**，选择&#x200B;**[!UICONTROL 添加用户]**，然后添加管理员。
1. 确认每个管理员也是`Marketo Qualifier`组的成员。

两个群组的成员身份使&#x200B;**[!UICONTROL 管理员设置]**&#x200B;在左侧导航栏的&#x200B;**[!UICONTROL 管理]**&#x200B;下可见。 标准用户使用管理员配置的字段、过滤器和剧本。 配置的选择退出页脚自动应用于其出站电子邮件。 标准用户无法更改这些设置。

用户组名必须与上一步骤中所示完全匹配。

您还可以创建可选的`Marketo Qualifier BDR managers`组。 此组的成员可以访问电子邮件性能报表。

## 连接您的CRM

Marketo Qualifier可连接到Salesforce或Microsoft Dynamics 365，从而为BDR提供用户、潜在客户、联系人、客户、商机、所有者映射和相关活动的统一视图。 初始连接需要此CRM数据的只读访问权限。 要在连接Marketo限定符之前准备凭据，请与您的CRM管理员联系。 有关集成详细信息，请参阅[集成](integrations.md)。

>[!PREREQUISITES]
>
>要访问CRM管理界面，您必须属于`Marketo Qualifier Admins` Adobe Admin Console组和`Marketo Qualifier`组。
...
1. 对Marketo限定符使用规范形式： `https://{{mydomain}}.my.salesforce.com`。
...
1. 作为两个必需的Marketo限定符组的成员，登录到Marketo限定符并确认选择了正确的沙盒或环境。
...
连接CRM后，配置入站映射以确定哪些CRM字段显示在Marketo限定符中。 在连接的CRM信息卡上，选择&#x200B;**[!UICONTROL 管理]**&#x200B;以打开&#x200B;**[!UICONTROL 入站映射]**，然后为要导入其字段的每个实体类型添加一个节。
