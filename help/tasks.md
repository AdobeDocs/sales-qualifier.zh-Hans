...
使用&#x200B;**[!UICONTROL 任务]**&#x200B;完成参与计划生成的操作。 选择任务，执行操作，将任务标记为“完成”，然后在不离开页面的情况下继续执行下一个任务。
...
* **[!UICONTROL 手动任务]** — 已注册参与计划的潜在客户的电话通话、LinkedIn InMails和电子邮件审核。
* **[!UICONTROL 代理建议]** — 符合参与计划的定位条件并建议注册的潜在客户。
...
* **[!UICONTROL 参与计划]** — 关联的参与计划名称、截止日期和自动跳过指示器（如果适用）。
* **任务内容** — 调用脚本或InMail内容。
...
* **[!UICONTROL 跳过]** — 当您无法完成步骤但希望将潜在客户保留在参与计划中时，请使用此操作。 前景进入下一个步调。
...
* **[!UICONTROL 筛选器]** — 打开筛选器面板：
  ...
  * **[!UICONTROL 到期日期]**。
  * **[!UICONTROL 参与计划]** — 参与计划的可搜索列表。
* **[!UICONTROL 排序]** — 按到期日期或创建日期排序。 排序顺序还决定了队列前进的顺序。
* **[!UICONTROL 搜索任务]** — 按潜在客户名称、公司名称或参与计划查找任务。 搜索适用于活动的过滤器。
...
| **[!UICONTROL 跳过]** | 您已跳过该步骤，或该步骤已自动跳过。 潜在客户在“参与计划”中前进。 |
| **[!UICONTROL 已取消]** | 由于预订计划更改，系统取消了任务。 |

已完成、已跳过和已取消的任务是最终任务。 他们的操作不再可用，并且其注释为只读。

## 代理建议

**[!UICONTROL 代理建议]**&#x200B;选项卡列出了与参与计划的定位条件匹配的潜在客户，建议注册。 要启用推荐，请参阅[出站工作流](outbound-workflows.md)。
...
* **[!UICONTROL 审核潜在客户]** — 打开参与计划以审核和注册推荐的潜在客户。 查看[添加潜在客户并开始生成电子邮件](outbound-workflows.md#step-5-add-prospects-and-start-email-generation)。
* **[!UICONTROL 标记为完成]** — 审阅建议后将其关闭。

**[!UICONTROL 代理建议]**&#x200B;选项卡包括&#x200B;**[!UICONTROL 当前]**、**[!UICONTROL 已完成]**&#x200B;和&#x200B;**[!UICONTROL 已取消]**&#x200B;状态筛选器、参与计划筛选器，以及按创建日期排序。

## 从参与计划中完成任务

在参与计划的&#x200B;**[!UICONTROL 参与的潜在客户]**&#x200B;视图中，手动接触点提供了相同的&#x200B;**[!UICONTROL 标记为完成]**、**[!UICONTROL 跳过]**&#x200B;和注释选项。 在该处完成任务也会在&#x200B;**[!UICONTROL 任务]**&#x200B;页面上更新其状态。 请参阅[出站工作流](outbound-workflows.md)。
