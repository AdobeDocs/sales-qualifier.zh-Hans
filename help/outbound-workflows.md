...
参与计划是一种目标驱动的外联频率。 您可以定义目标和定位标准。 然后，AI提出一种多点触控节奏，并为每个潜在客户编写个性化的电子邮件内容。 在激活节奏之前，请查看并批准每封电子邮件。
...
| 概念 | 描述 |
| --- | --- |
| **参与计划** | 由目标、定向过滤器、节奏和设置定义的可重用出站活动。 |
| **目标** | 外联工作应该取得什么成果。 |
| **接触点** | 按计划相对于注册步调执行一步（电子邮件、电话或链接In Mail）。 |
| **接触点提示** | 在生成潜在客户的电子邮件主题行和正文时，AI将遵循相关说明，包括语调、时长、焦点和call to action。 |
| **节奏** | 接触点的完整序列：数量、顺序以及日期。 |
| **定位筛选器** | 将参与计划限制为潜在客户子集的条件。 |
| **草稿** | 已生成准备好进行审查但尚未批准的电子邮件。 |
| **推理** | AI对如何写一封给定电子邮件的解释，包括它使用的信号和数据源。 |
| **注册** | 批准目标客户的草稿，这将激活节奏并在参与计划的发送窗口内将要发送的电子邮件排入队列。 |

以下部分将介绍如何创建参与计划、审查生成的电子邮件、批准潜在客户和管理出站工作流。

## 创建参与计划

参与计划向导有五个步骤：**[!UICONTROL 目标]**、**[!UICONTROL 定位]**、**[!UICONTROL 生成接触点]**、**[!UICONTROL 设置]**&#x200B;和&#x200B;**[!UICONTROL 添加潜在客户]**。 您的目标决定了剩余的步骤。

1. 在左侧导航中，选择&#x200B;**[!UICONTROL 出站工作流]**。
1. 在&#x200B;**[!UICONTROL 浏览]**&#x200B;选项卡的右上角选择&#x200B;**[!UICONTROL +创建参与计划]**。
...
**[!UICONTROL 设置]**&#x200B;步骤控制参与计划的运行方式。

1. 查看&#x200B;**[!UICONTROL 参与计划名称]**，并根据需要更改它。
1. 在&#x200B;**[!UICONTROL 每个参与计划的最大潜在客户数]**&#x200B;中，确认参与计划可以同时管理的最大潜在客户数。
...
1. 选择使用每个潜在客户的时区还是参与计划&#x200B;**[!UICONTROL 时区]**&#x200B;进行发送计时。 如果您使用参与计划时区，请确认它与受众匹配。
1. 在&#x200B;**[!UICONTROL 权限]**&#x200B;下，保留&#x200B;**[!UICONTROL 私有]**（默认）或选择&#x200B;**[!UICONTROL 与所有人共享]**。 有关详细信息，请参阅[共享参与计划](#share-an-engagement-plan)。
1. 选择&#x200B;**[!UICONTROL 保存并添加潜在客户]**。

选择退出页脚由管理员全局配置，并独立于参与计划设置应用于出站电子邮件。 请参阅[配置全局电子邮件选择退出](integrations.md#configure-global-email-opt-out)。
...
完成生成后，“参与计划”详细信息视图将提示您复查草稿。 在您批准之前，Sales Qualifier不会发送电子邮件。

1. 在参与计划详细信息视图中，选择横幅中的&#x200B;**[!UICONTROL 审核草稿]**。
...
根据参与计划的选定日期、发送窗口、活动小时选项和时区设置发送批准的电子邮件。 零延迟的接触点无需等待即可发送；其他每个接触点都遵循其配置的延迟。 未批准的潜在客户保留在&#x200B;**[!UICONTROL 中可供审查]**。

## 共享参与计划

每个参与计划都有&#x200B;**[!UICONTROL 权限]**&#x200B;设置。 出站工作流默认为&#x200B;**[!UICONTROL 私有]**。 所有者可以选择&#x200B;**[!UICONTROL 与所有人共享]**&#x200B;以使参与计划可供团队使用。

>[!CAUTION]
>
>共享是永久性的。 将参与计划设置为&#x200B;**[!UICONTROL 与所有人共享]**&#x200B;后，无法将其更改回&#x200B;**[!UICONTROL 私有]**。

在共享的参与计划中，队友可以注册自己的潜在客户。 每个人员只能管理或暂停他们注册的潜在客户，包括使用批量操作时。 仅参与计划所有者可以编辑计划级别的设置，包括计划、时区、节奏和其他设置。 这些设置对于队友是只读的。
...
当潜在客户回复邮件外出邮件时，“参与计划”会自动处理该邮件。

* **自动恢复**：默认开启。 如果外出回复包括返回日期，则参与计划会恢复该日期的频率。 如果未提供返回日期，则参与计划在您的团队可以配置的恢复后缓冲区后恢复。
* **手动选项**：您还可以手动恢复、暂停或跳过潜在客户。 请参阅[管理现有的出站工作流](#manage-existing-engagement-plans)。

## 管理现有出站工作流

在&#x200B;**[!UICONTROL 出站工作流]**&#x200B;页面上，**[!UICONTROL 浏览]**&#x200B;选项卡列出了您可以使用的每个参与计划。 每个信息卡都会显示目标、配置的接触点和性能指标。 使用此视图监视出站工作流、审查草稿或添加潜在客户。
...
* **Booking暂停** — 当潜在客户预订会议时，**[!UICONTROL Meeting Booking暂停]**&#x200B;停止后续跟进。 请参阅[步骤4：配置参与计划设置](#step-4-configure-engagement-plan-settings)。

在[出站性能](performance.md)页面上跟踪预订结果。

## 参与计划最佳实践

* **定义特定目标。** 定位、节奏和电子邮件都源自目标。 说明您希望参与计划实现的结果。
